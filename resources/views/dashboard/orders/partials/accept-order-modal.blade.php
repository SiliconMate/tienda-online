<x-modal name="accept-order-{{ $order->id }}" :show="$errors->has('completeOrder')" focusable>
    <form action="{{ route('dashboard.orders.accept', $order->id) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="grid grid-cols-1 gap-3">
            <div class="grid grid-cols-1 p-6 pb-0">
                <div class="flex items-center">
                    <i class="ti ti-check text-3xl text-gray-800 pr-2"></i>
                    <h2 class="font-semibold text-xl text-gray-800 leading-tight">Pedido Enviado</h2>
                </div>
            </div>
