            ...
            <div class="flex items-center mb-4">
                <form method="GET" action="{{ route('products.index') }}" id="sortForm">
                    ...
                    <select name="sort" id="sort"
                        ...
                        <option value="latest" {{ request('sort') == 'latest' ? 'selected' : '' }}>Últimos productos</option>
                        <option value="price-low-to-high" {{ request('sort') == 'price-low-to-high' ? 'selected' : '' }}>Menor precio</option>
                        <option value="price-high-to-low" {{ request('sort') == 'price-high-to-low' ? 'selected' : '' }}>Mayor precio</option>
                    </select>
                </form>
            </div>
            ...
            <div class="grid md:grid-cols-3 grid-cols-2 gap-6">
                ...
                <div class="bg-white shadow rounded overflow-hidden group flex flex-col justify-between transition-transform transform hover:scale-105">
                    ...
                    <div class="relative h-64 flex items-center justify-center bg-gray-100">
                        ...
                        <img src="{{ asset('storage/products/' . $product->images->first()->path) }}" alt="{{ $product->name }}" class="max-h-full max-w-full object-contain">
                        </a>
                    </div>
