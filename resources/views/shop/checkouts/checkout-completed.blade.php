<x-home-layout>
    ...
    <div class="bg-gray-200 border border-gray-300 shadow-lg rounded-lg p-8 mb-6 mx-8">
        ...
        <form action="{{ route('opinions.store') }}" method="POST">
            @csrf
            {{-- <input type="hidden" name="product_id" value="{{ $orderDetail->orderItems->first()->product_id }}">
            <input type="hidden" name="user_id" value="{{ $orderDetail->user->id }}">
             --}}
            <div class="mb-6">
                <label for="content" class="block text-lg font-medium text-gray-700 mb-2">Tu opinión</label>
                <textarea id="content" name="content" rows="4" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" required></textarea>
            </div>
            
            <div class="mb-6">
                <label for="rating" class="block text-lg font-medium text-gray-700 mb-2">Calificación</label>
                <div class="flex items-center justify-center">
                    @for ($i = 1; $i <= 5; $i++)
                        <input type="radio" id="rating-{{ $i }}" name="rating" value="{{ $i }}" class="hidden" required>
                        <label for="rating-{{ $i }}" class="cursor-pointer">
                            <svg class="w-10 h-10 text-gray-400 hover:text-yellow-500 star transition duration-200" fill="currentColor" viewBox="0 0 20 20" data-value="{{ $i }}">
